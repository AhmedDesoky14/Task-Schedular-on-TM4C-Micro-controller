 ...
 * File Name: Dio.c
 *
 * Description: Source file for TM4C123GH6PM Micro-controller - Dio Driver
 *
 * Author: Ahmed Desoky
 ************************************************************************************************************/
...
#if(DIO_CONFIGURED_PORTS > 0)
		Dio_Ports = ConfigPtr->Ports;	/* Address of the first port - Dio_PortType	Ports[0] */
#endif
	}
}
...
{
    volatile uint32 * Port_Ptr = NULL_PTR;
    Dio_LevelType ReadLevel = STD_LOW;
    boolean error = FALSE;
    #if (DIO_DEV_ERROR_DETECT == STD_ON)
    /* Check if the Driver is initialized before using this function */
	if (DIO_NOT_INITIALIZED == Dio_Status)
	...
	if(FALSE == error)
	{
          /* Point to the correct PORT register according to the Port Id stored in the Port_Num member */
            ...
            /*check Channel is SET or NOT*/
            if(BIT_IS_SET(*Port_Ptr,Dio_PortChannels[ChannelId].Ch_Num))
            {
              ReadLevel = STD_HIGH;
            }
            else if(BIT_IS_CLEAR(*Port_Ptr,Dio_PortChannels[ChannelId].Ch_Num))
            {
              ReadLevel = STD_LOW;
            }
        }
...
{
	/* Pointer to store Port Registers base address */
	volatile uint32 *Port_Ptr = NULL_PTR;
	Dio_PortLevelType ReadValue = STD_LOW;
	boolean error = FALSE;
#if (DIO_DEV_ERROR_DETECT == STD_ON)
    /* Check if the Driver is initialized before using this function */
	if (DIO_NOT_INITIALIZED == Dio_Status)
	...
	if(FALSE == error)
	{
		...
		ReadValue = (uint8)((*Port_Ptr) & (0x000000FF));
	}
	else
	{
		/* Do Nothing */
...
* Reentrancy: Reentrant
* Parameters (in): PortId - ID of DIO port.
*                  PortLevel - Value to be written.
* Parameters (inout): None
* Parameters (out): None
* Return value: None
* Description: Function to set a level of a DIO port.
******************************************************************************************************************/
void Dio_WritePort(Dio_PortType PortId, Dio_PortLevelType Value)
{
	/* Pointer to store Port Registers base address */
	volatile uint32 *Port_Ptr = NULL_PTR;
	boolean error = FALSE;
